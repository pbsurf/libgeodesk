...
/*
template<>
void BlobStore::CreateTransaction<BlobStore>::begin(const char* filename)
{
    store_.ExpandableMappedFile::open(filename,
        File::OpenMode::READ | File::OpenMode::WRITE | File::OpenMode::CREATE);
    BlobStore::Transaction::begin(LOCK_EXCLUSIVE);
    HeaderBlock* header = getRootBlock();
    header->magic = MAGIC;
    header->versionHigh = 2;
    header->versionLow = 0;
    header->creationTimestamp = DateTime::now();
    header->pageSize = 12; // TODO: default
...
void BlobStore::initHeader(HeaderBlock* header)
{
    header->magic = MAGIC;
    header->versionHigh = 2;
    header->versionLow = 0;
